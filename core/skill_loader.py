"""
HebutGuide Skill 加载器。

Skill 是一段可热加载的招生咨询业务规则，用来补充对应 Agent 的 system prompt。
它适合放置报考口径、信息澄清流程、风险提示、专业规划边界和官方升级规则。
"""
import json
import logging
...
class Skill:
    ...
    def matches(self, message: str, agent_type: Optional[str] = None) -> bool:
        ...
        if self.agents:
            ...
            if normalized_agent not in self.agents:
                return False
...
class SkillManager:
    ...
    def prompt_for(self, message: str, agent_type: Optional[str] = None) -> str:
        ...
        return (
            "以下是当前请求可用的 HebutGuide 招生咨询 Skills。"
            "请优先遵循这些业务规则；如果与系统角色、知识库或 MCP 工具结果冲突，"
            "以系统角色、知识库、MCP 工具结果和官方安全边界为准。\n\n"
            + "\n\n".join(blocks)
        )
